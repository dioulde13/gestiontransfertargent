...
const ajoutSoldePartenaire = async (req, res) => {
  try {
    ...
    if (devise.signe_2 === "XOF") {
      ...
      utilisateur.solde_echange = Number(utilisateur.solde_echange || 0) + Number(montant);
      await utilisateur.save();

      partenaire.montant_credit = Number(partenaire.montant_credit || 0) + Number(montant);
      await partenaire.save();
    } else if (devise.signe_2 === "USD") {
      utilisateur.sign_dollar = devise.signe_2;  // Assigner uniquement la devise
      utilisateur.solde_echange_dollar = Number(utilisateur.solde_echange_dollar || 0) + Number(montant);
      await utilisateur.save();

      partenaire.montant_credit_dollar = Number(partenaire.montant_credit_dollar || 0) + Number(montant);
      await partenaire.save();
    } else if (devise.signe_2 === "EUR") {
      utilisateur.sign_euro = devise.signe_2;
      utilisateur.solde_echange_euro = Number(utilisateur.solde_echange_euro || 0) + Number(montant);
      await utilisateur.save();

      partenaire.montant_credit_euro = Number(partenaire.montant_credit_euro || 0) + Number(montant);
      await partenaire.save();
    }
...
const modifier = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: 'Partenaire mis à jour avec succès', partenaire }); 
...
module.exports = { ajouterEchange, recupererEchange, compterEchangeDuJour, ajoutSoldePartenaire ,modifier};
